...
export function ChoiceButton({
  icon, label, sublabel, selected, onPress, delay,
}: {
  icon:      string;
  label:     string;
  sublabel?: string;
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      <TouchableOpacity
        style={[cb.btn, selected && cb.btnSelected]}
        onPress={handlePress}
        activeOpacity={0.85}
      >
        ...
        {selected && (
          <LinearGradient
            colors={['rgba(255,144,48,0.22)', 'rgba(255,94,14,0.10)']}
        ...
        <View style={cb.textWrap}>
          <Text style={[cb.label, selected && cb.labelSelected]}>{label}</Text>
          {sublabel && <Text style={cb.sublabel}>{sublabel}</Text>}
        </View>

        {/* Check circle */}
        <Animated.View style={[cb.check, { opacity: checkA }]}>
          <LinearGradient colors={['#FF9030', '#FF5E0E']} style={cb.checkGrad}>
            <Text style={cb.checkMark}>✓</Text>
          </LinearGradient>
        </Animated.View>
        {!selected && <View style={cb.checkEmpty} />}
      </TouchableOpacity>
    </Animated.View>
  );
}
