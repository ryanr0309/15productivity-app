...
export default function QPhoneScreen() {
  ...
  // Track position at the moment the finger lands — dx is always relative to
  // this, so there's no velocity compounding between Move calls.
  const posAtDragStart = useRef(hoursToPos(4) * SLIDER_TRACK);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (e: GestureResponderEvent) => {
        ...
        const tapX = e.nativeEvent.locationX - 16; // account for trackWrap paddingHorizontal
        updateSlider(tapX);
        // Record where the thumb is NOW so Move deltas are relative to this
        posAtDragStart.current = currentX.current;
        Animated.spring(thumbScale, { toValue: 1.35, tension: 100, friction: 5, useNativeDriver: true }).start();
      },

      onPanResponderMove: (_: GestureResponderEvent, g: PanResponderGestureState) => {
        // g.dx is total displacement from the Grant point — not a delta per frame.
        // Adding it to posAtDragStart gives us a stable, linear relationship
        // between finger travel and thumb travel. No compounding. No sensitivity.
        updateSlider(posAtDragStart.current + g.dx);
      },
    ...
    >
      <View style={sl.wrap}>
        ...
        <View style={sl.trackWrap} {...panResponder.panHandlers}>
          ...
          >
            <LinearGradient colors={['#FF9030', '#FF5E0E']} style={sl.thumbGrad} />
...
const ql = StyleSheet.create({
  ...
  progressWrap: { flex: 1 },
  header: { paddingHorizontal: 28, paddingBottom: 24, gap: 10 },
  eyebrow: {
  ...
  question: {
    ...
    color: COLORS.cream, lineHeight: 38, letterSpacing: -0.5,
  },
  body: { flex: 1, paddingHorizontal: 28, gap: 10 },
  ctaWrap: { paddingHorizontal: 28, paddingTop: 16 },
  ctaBtn: {
...
const cb = StyleSheet.create({
  ...
  check: { width: 26, height: 26, borderRadius: 13, overflow: 'hidden' },
...
const sl = StyleSheet.create({
  ...
  bigTime: { fontFamily: FONTS.black, fontSize: 64, letterSpacing: -2, lineHeight: 72 },
  bigTimeSub: {
    fontFamily: FONTS.regular, fontSize: 14,
    color: 'rgba(255,244,230,0.35)', letterSpacing: 1, textTransform: 'uppercase',
  },
  ...
  trackWrap: { height: 44, justifyContent: 'center', paddingHorizontal: 16 },
  track: {
    height: 6, backgroundColor: 'rgba(255,255,255,0.10)',
    borderRadius: 3, overflow: 'visible',
  },
  fill: { height: 6, borderRadius: 3, overflow: 'hidden', minWidth: 6 },
  thumb: {
    position: 'absolute', width: 28, height: 28, borderRadius: 14,
    marginLeft: 16 - 14, top: 44 / 2 - 14, overflow: 'hidden',
  },
  ...
  scaleLabel: { fontFamily: FONTS.regular, fontSize: 11, color: 'rgba(255,244,230,0.22)' },
  scaleLabelActive: { color: COLORS.amber, fontFamily: FONTS.bold },
  projCard: {
  ...
  projLabel: {
    fontFamily: FONTS.regular, fontSize: 12,
    color: 'rgba(255,244,230,0.35)', textTransform: 'uppercase', letterSpacing: 1,
  },
  projNumber: { fontFamily: FONTS.black, fontSize: 22, color: COLORS.cream, lineHeight: 28 },
  projBar: { height: 5, backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 3, overflow: 'hidden' },
