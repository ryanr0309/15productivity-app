...
const PROTECT_OPTIONS = [
  { icon: '💼', label: 'Deep work',         sublabel: 'Code, writing, design, strategy'   },
  { icon: '📚', label: 'Studying',          sublabel: 'Exams, learning, courses'           },
  { icon: '🎨', label: 'Creative projects', sublabel: 'Art, music, video, side projects'   },
  { icon: '🏃', label: 'Health & fitness',  sublabel: 'Workouts, meal prep, sleep routines' },
  { icon: '👨‍👩‍👧', label: 'Family & presence', sublabel: 'Being actually here with people I love' },

];

export default function QProtectScreen() {
  ...
  const handleContinue = () => {
    if (selected) {
      setAnswer('protectTime', selected);
   ...
   const posthog = usePostHog()
    ...
    useEffect(() => {
      posthog.capture('onboarding_step_viewed', { step: 'protect' })
    }, [])
  ...
  return (
    ...
    >
      ...
      <Text style={choices.hint}>This shapes your personalized plan</Text>
    </QuestionLayout>
  );
}
