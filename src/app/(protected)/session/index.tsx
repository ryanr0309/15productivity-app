...
export default function SessionScreen() {
  ...
  // Navigate home when session ends naturally (timer hits zero)
  useEffect(() => {
    if (!isRunning && elapsed > 0 && elapsed >= durationSec) {
  ...
  }, [isRunning, elapsed, durationSec, resetSession]);
  ...
  // When focused, scroll to end so goal line + input are both above keyboard
  ...
  return (
    <KeyboardAvoidingView
      style={{ flex: 1, backgroundColor: '#090602' }}
    ...
    >
      ...
      >
        <Animated.View style={{ opacity: fadeIn, alignItems: 'center', width: '100%' }}>
          ...
          {/* ── Goal — sits just above exit input so it's visible while typing ── */}
          ...
          <View style={styles.cpSection}>
            ...
            <Animated.View style={{
              opacity: cpOpacity,
              transform: [{ scale: cpScale }, { scale: cpPulse }],
              pointerEvents: (isRunning && checkpointReady) ? 'auto' : 'none',
            }}>
              <TouchableOpacity style={styles.cpBtn} activeOpacity={0.85} onPress={handleCheckpointPress}>
                <LinearGradient
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.cpBtnSub}>Take a 5-min break</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  mascotWrap:    { position: 'absolute', alignItems: 'center', justifyContent: 'center' },
  mascotGlow:    { position: 'absolute', width: 90, height: 90, borderRadius: 45, backgroundColor: '#FF6B1A' },
  ...
  // Exit hatch
  exitSection:   { width: '100%', alignItems: 'center', paddingTop: 8, paddingBottom: 16 },
